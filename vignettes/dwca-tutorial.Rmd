...
occurrence_df <- data_occurrences %>% 
  select(occurrenceID, occurrenceStatus, collectionCode, type, basisOfRecord,
         identificationVerificationStatus, identificationReferences, 
         identifiedBy, associatedMedia, eventID, eventDate, scientificName, 
         scientificNameAuthorship, scientificNameID, taxonRank, kingdom, 
         phylum, class, order, family, genus, verbatimIdentification)
...
```{r}
# Convert biovolume units and select the relevant columns for occurrence eMoF
data_occurrence_emof <- data_occurrence_emof %>% 
  mutate(biovolume_um3_per_liter = biovolume_mm3_per_liter * 10^9) %>%
  select(parentEventID, eventID, occurrenceID, counts, 
...
data_parent_emof <- data_parent_emof %>%
  select(parentEventID, eventID, occurrenceID, instrument, ifcb_number)

# Select the relevant columns for event eMoF
data_event_emof <- data_event_emof %>% 
  select(parentEventID, eventID, occurrenceID, ml_analyzed)
```
...
# Combine all standardized measurements into a single dataset and add unique measurementIDs
