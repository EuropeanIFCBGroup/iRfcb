...
```{r}
# Summarize manual biovolume data
...
Additionally, each Parent Event is associated with specific date ranges, which must be captured as **eventDate** to reflect the temporal span of the observations or data collection. This helps provide clear temporal context for the data.
...
To organize our data effectively, we start by defining key event terms. These include general terms like **eventType** and **ownerInstitutionCode**. Each term represents specific metadata associated with the collected data. Below is the annotated R code that processes and structures the data into an event-focused format.

```{r}
# Add metadata columns to the data
data_event <- data_event %>%
  mutate(
    # Defining the institution responsible for the data
    ownerInstitutionCode = "SMHI",
    
    # Specifying the type of record, which is Event in this case
    eventType = "Sample",
...
event_df <- data_event %>%
  select(eventType, ownerInstitutionCode, parentEventID, eventID,
    samplingProtocol, sampleSizeValue, sampleSizeUnit, eventDate, eventTime,
    year, month, day, country, countryCode, decimalLatitude, decimalLongitude,
...
The Occurrence table captures information about individual organisms or observations, linking them to a specific event. For IFCB data, the **basisOfRecord** indicates how the observation was made. Since this example uses manually annotated images, the **basisOfRecord** is set to **MachineObservation** and the identificationVerificationStatus is set to **ValidatedByHuman**. For best practices in plankton imaging data management, see [Martin-Cabrera et al. (2022)](https://dx.doi.org/10.25607/OBP-1742).

### Annotated Code: Define Occurrence Data

Each row of the IFCB data is considered an occurrence.
...
data_occurrences <- data_event %>%  
  ...
  mutate(  
    ...
    collectionCode = "iRfcb IFCB Test Dataset",  # Provides a collection identifier  
...
The **scientificName** and **vernacularName** fields are populated using the cleaned taxonomic names and the original class names, respectively.

```{r}
data_occurrences <- data_occurrences %>% 
  rename(class_name = class) %>%
  left_join(class_names, by = "class_name") %>%
  mutate(scientificName = if_else(is.na(scientificname),
                                  class_clean, 
                                  scientificname),
         scientificNameAuthorship = authority,
         vernacularName = class_name,
...
occurrence_df <- data_occurrences %>% 
  select(type, collectionCode, basisOfRecord, occurrenceID, organismQuantity, 
         organismQuantityType, occurrenceStatus, identificationVerificationStatus, 
         identificationReferences, identifiedBy, associatedMedia, eventID, eventDate, 
         scientificName, scientificNameAuthorship, scientificNameID, kingdom, phylum, 
         class, order, family, genus, vernacularName, taxonRank)
...
The eMoF table allows us to capture any additional measurements and facts associated with occurrences, such as biological or environmental measurements. For IFCB data, we can include information such as counts, abundance, biovolume concentration, and carbon content. These measurements provide essential context for understanding the ecological significance of the observations.

First, we generate a unique measurementID for each row using the `uuid::UUIDgenerate` function to ensure traceability.

```{r}
# Generate unique measurement IDs for each occurrence
data_occurrence_emof <- data_occurrences %>% 
  mutate(measurementID = uuid::UUIDgenerate(use.time = FALSE))

# Generate unique measurement IDs for each event
data_event_emof <- data_event %>% 
  mutate(measurementID = uuid::UUIDgenerate(use.time = FALSE),
         occurrenceID = NA,)

# Generate unique measurement IDs for each parent event
data_parent_emof <- data_parent_event %>% 
  mutate(measurementID = uuid::UUIDgenerate(use.time = FALSE),
         occurrenceID = NA,
         instrument = "IFCB")
```

Next, we extract the necessary columns from the dataset that will be used in the eMoF table. This includes the measurement ID, associated event and occurrence IDs, and key IFCB-derived measurements such as counts, abundance, biovolume, and carbon concentration.

```{r}
# Select the relevant columns for occurrence eMoF
data_occurrence_emof <- data_occurrence_emof %>% 
  select(measurementID, parentEventID, eventID, occurrenceID,
         counts, counts_per_liter, biovolume_mm3_per_liter, carbon_ug_per_liter)

# Select the relevant columns for parentEvent eMoF
data_parent_emof <- data_parent_emof %>%
  select(measurementID, parentEventID, eventID, occurrenceID, instrument, ifcb_number)

# Select the relevant columns for event eMoF
data_event_emof <- data_event_emof %>% 
  select(measurementID, parentEventID, eventID, occurrenceID, ml_analyzed) %>%
  distinct()
```
...
data_occurrence_emof <- data_occurrence_emof %>% 
  pivot_longer(cols = c(counts, counts_per_liter, biovolume_mm3_per_liter, carbon_ug_per_liter), 
               names_to = "measurementType",
               values_to = "measurementValue") %>% 
  drop_na(measurementValue) %>%
  mutate(measurementType = gsub("counts_per_liter", "Abundance", measurementType)) %>%
  mutate(measurementType = gsub("biovolume_mm3_per_liter", "Biovolume concentration", measurementType)) %>%
...
# Combine all standardized measurements into a single dataset
data_emof <- bind_rows(data_parent_emof, data_event_emof, data_occurrence_emof)
```
...
nerc_vocab <- data.frame(
  ...
  measurementTypeID = c("http://vocab.nerc.ac.uk/collection/P01/current/OCOUNT01/",
                        ...
                        rep(NA, 2)),
  measurementUnit = c("Individual per analysed sample fraction", 
                      "Individual per litre", 
                      "Cubic millimetres per litre", 
...
# Merge the data with NERC vocabularies
emof_df <- data_emof %>%
...
```{r}
GBIF_eMoF <- initializeGBIFMeasurementOrFact(emof_df, 
...
In this step, the finalized tables (`GBIF_Event`, `GBIF_Occurrence`, and `GBIF_eMoF`) and the metadata (`GBIF_Metadata`) are bundled together into a DwC-A `.zip` file that is ready for submission to platforms like GBIF using an Integrated Publishing Toolkit (IPT), e.g. the [EurOBIS IPT](https://ipt.vliz.be/eurobis/).
...
dwca_archive <- initializeDwCArchive(GBIF_Event, 
                                     list(GBIF_Occurrence, GBIF_eMoF), 
                                     GBIF_Metadata)
