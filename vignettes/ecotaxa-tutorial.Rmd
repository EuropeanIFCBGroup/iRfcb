...
This vignette demonstrates how to prepare Imaging FlowCytobot (IFCB) data for [EcoTaxa](https://ecotaxa.obs-vlfr.fr/) in R using the `iRfcb` package. This tutorial covers the export of both unclassifie raw IFCB images and annotated Regions of Interest (ROIs) using the MATLAB code from the [ifcb-analysis](https://github.com/hsosik/ifcb-analysis) repository (Sosik and Olson, 2007). However, the code can be adapted to process images from other software platforms. The code can also be adapted to submit automatically classified images using the [`ifcb_extract_classified_images`](../reference/ifcb_extract_classified_images.html) function.
...
This example demonstrates how to prepare a single unannotated IFCB sample for submission to EcoTaxa as a zip-archive.

### Extract Images

Extract all ROIs from a sample as `.jpg` images:
...
Extract image metadata from the `.jpg` directory:
```{r}
...
This example demonstrates how to prepare a dataset of annotated images for submission to EcoTaxa using the MATLAB code from the [ifcb-analysis](https://github.com/hsosik/ifcb-analysis) repository (Sosik and Olson, 2007).
