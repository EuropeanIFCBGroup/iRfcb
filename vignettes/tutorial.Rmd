...
This function calculates aggregated biovolumes and carbon content from manually annotated Imaging FlowCytobot (IFCB) images based on feature files and manual MATLAB files. Biovolumes are converted to carbon according to Menden-Deuer and Lessard (2000) for individual regions of interest (ROI), where different conversion factors are applied to diatoms and non-diatom protist. If provided, it also incorporates sample volume data from HDR files to compute biovolume and carbon content per liter of sample. See details in the help pages for `ifcb_count_mat_annotations`, `ifcb_summarize_biovolumes` and `ifcb_extract_biovolumes`.
