...
Both functions return a data frame with `file_name`, `class_name`, and `score` columns, and query the Gradio API at `https://ifcb.serve.scilifelab.se` by default. The `top_n` argument controls how many top predictions are returned per image.
