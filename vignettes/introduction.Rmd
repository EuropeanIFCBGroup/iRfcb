...
If images have already been extracted, pass a vector of PNG file paths to `ifcb_classify_image()`:
...
# Classify images
results <- ifcb_classify_image(png_files, verbose = FALSE)

# Print result
print(results)
```

Both functions return a data frame with `file_name`, `class_name`, `score`, and `model_name` columns, and query the Gradio API at `https://irfcb-classify.hf.space` by default. The `top_n` argument controls how many top predictions are returned per image, and `model_name` specifies which CNN model to use (default: `"SMHI NIVA ResNet50 V5"`).

### Apply Per-Class Thresholds

Set `apply_threshold = TRUE` to add a `class_name_above_threshold` column. Predictions scoring below the per-class F2 optimal threshold are labelled `"unclassified"`:
...
### Save Classification as HDF5

`ifcb_save_classification()` classifies all images in a `.roi` file and saves the full score matrix in the IFCB Dashboard v3 HDF5 format (requires the `hdf5r` package):
