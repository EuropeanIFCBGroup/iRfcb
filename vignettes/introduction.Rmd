...
This requires the optional `ifcb-features` package, which is installed alongside its dependencies (`pyifcb`, `phasepack`, `scikit-image`, `scikit-learn`) by passing `features = TRUE` to `ifcb_py_install()`:
