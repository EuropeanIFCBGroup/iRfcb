...
IFCB images can be classified directly in R using a CNN model served by a [Gradio](https://www.gradio.app/) application.
...
Both functions return a data frame with `file_name`, `class_name`, `score`, and `model_name` columns, and query the Gradio API at `https://ifcb.serve.scilifelab.se` by default. The `top_n` argument controls how many top predictions are returned per image, and `model_name` specifies which CNN model to use (default: `"SMHI NIVA ResNet50 V5"`).
