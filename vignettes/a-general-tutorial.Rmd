...
print(timestamps)
```
If the filename includes ROI numbers (e.g., in an extracted `.jpg` image), a separate column, `roi`, will be added to the output.
...
IFCB images stored in `.roi` files can be extracted as `.jpg` files using the `iRfcb` package, as demonstrated below.

Extract all images from a sample using the [`ifcb_extract_pngs`](../reference/ifcb_extract_pngs.html) function. You can specify the `out_folder`, but by default, images will be saved in a subdirectory within the same directory as the ROI file.
```{r}
# All ROIs in sample
