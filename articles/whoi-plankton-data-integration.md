...
The [WHOI-Plankton](https://hdl.handle.net/1912/7341) dataset (Sosik et
al. 2015) contains millions of microscopic marine plankton images
captured by the IFCB, and manually classified into \> 100 categories by
researchers at the Woods Hole Oceanographic Institution (WHOI). The
dataset is part of a larger collection of over 700 million images
gathered since 2006 at the Martha’s Vineyard Coastal Observatory (MVCO),
with real-time and archived data accessible via the [IFCB Data
Dashboard](https://ifcb-data.whoi.edu/mvco). Annotated images are stored
in annual zip files, organized into class-specific subdirectories.
Example images for each class are available on the WHOI-Plankton [GitHub
page](https://whoigit.github.io/whoi-plankton/).

`iRfcb` provides functions to interact with the WHOI-Plankton data and
the IFCB Dashboard, making it easier to integrate annotated images into
custom training datasets. It supports incorporation of these images
using MATLAB code from the
[`ifcb-analysis`](https://github.com/hsosik/ifcb-analysis) repository
(Sosik and Olson 2007), as demonstrated in this tutorial.
...
(Torstensson et al. 2024). This dataset will serve as the primary
dataset in this example, which we will expand by incorporating training
...
[`help("ifcb_prepare_whoi_plankton")`](https://europeanifcbgroup.github.io/iRfcb/reference/ifcb_prepare_whoi_plankton.md)

The
[`ifcb_prepare_whoi_plankton()`](https://europeanifcbgroup.github.io/iRfcb/reference/ifcb_prepare_whoi_plankton.md)
function enables downloading zipped `.png` images from selected years of
the [WHOI-Plankton dataset](https://hdl.handle.net/1912/7341), with an
option to extract them using the `extract_images` argument. It also
retrieves raw data (`.roi`, `.hdr` and `.adc` files), along with blobs
and features (when available), from the [MVCO IFCB
Dashboard](https://ifcb-data.whoi.edu/mvco). Interactions with
WHOI-Plankton are managed through the
[`ifcb_download_whoi_plankton()`](https://europeanifcbgroup.github.io/iRfcb/reference/ifcb_download_whoi_plankton.md)
function, while the IFCB Dashboard is accessed via the
[`ifcb_download_dashboard_data()`](https://europeanifcbgroup.github.io/iRfcb/reference/ifcb_download_dashboard_data.md)
function. You can also interact with the IFCB Dashboard using the
functions
...
information, ensuring compatibility with the MATLAB code in the
[`ifcb-analysis`](https://github.com/hsosik/ifcb-analysis) repository
...
To exclude certain images from the training dataset, either exclude the
class completely with the `skip_classes` argument, or set
`extract_images = TRUE` and manually delete specific `.png` files from
the `png_folder` and rerun `ifcb_prepare_whoi_plankton`.

The function
[`ifcb_prepare_whoi_plankton()`](https://europeanifcbgroup.github.io/iRfcb/reference/ifcb_prepare_whoi_plankton.md)
converts the filename format of older IFCB models (IFCB1-6) to match the
newer IFCB format (`DYYYYMMDDTHHMMSS_IFCBXXX`). By default, this
conversion is performed using the `convert_filenames` and `convert_adc`
arguments. This step ensures compatibility with the
[`ifcb-analysis`](https://github.com/hsosik/ifcb-analysis) MATLAB code.
For further details, please refer to the help pages of the respective
functions.
...
function allows you to integrate data from the WHOI-Plankton dataset
with your existing training dataset created using the
[`ifcb-analysis`](https://github.com/hsosik/ifcb-analysis) code (Sosik
and Olson 2007). The merged dataset and class2use file will be saved in
the locations specified by `manual_folder_output` and
`class2use_file_output`.

In this example, we merge the WHOI-Plankton dataset with the `iRfcb`
sample dataset from the [SMHI IFCB Plankton Image Reference
Library](https://doi.org/10.17044/scilifelab.25883455) (Torstensson et
al. 2024), downloaded earlier in this tutorial. Please note that class
names in the `class2use` files may need to be standardized if you intend
...
machine learning models. To integrate it with the
[`ifcb-analysis`](https://github.com/hsosik/ifcb-analysis) MATLAB code,
blobs and features are extracted for the merged dataset (if not already
downloaded through the IFCB Dashboard). Random forest models can then be
built using the
[`ifcb-analysis`](https://github.com/hsosik/ifcb-analysis) code.
...
## Citation
    ...
    ##   FlowCytobot (IFCB) Data. R package version 0.10.0.
...
- Sosik, H. M., Peacock, E. E. and Brownlee E. F. (2015), Annotated
  Plankton Images - Data Set for Developing and Evaluating
  Classification Methods. <https://doi.org/10.1575/1912/7341>
