...
This function reads a classified sample file (`.mat`, `.h5`, or `.csv`)
and extracts specified taxa images from the corresponding ROI files,
saving each image in a specified directory. Supports `.mat` files
generated by `start_classify_batch_user_training` from the
`ifcb-analysis` repository (Sosik and Olson 2007), `.h5` files in IFCB
Dashboard class_scores format, and `.csv` files in
`ClassiPyR`-compatible format.
