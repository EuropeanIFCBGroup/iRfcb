...
- n_cores:
  ...
  An integer specifying the number of worker processes to use when
  `parallel = TRUE`. If `NULL` (default), `parallel::detectCores() - 1`
...
Bins are processed sequentially by default. When `parallel = TRUE`, bins
are distributed across `n_cores` worker processes on the Python side,
which can substantially reduce runtime for large datasets. Existing
outputs are skipped unless `overwrite = TRUE`, so the function can be
re-run to resume an interrupted extraction.
