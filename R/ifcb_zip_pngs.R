...
ifcb_zip_pngs <- function(png_folder, zip_filename, readme_file = NULL, email_address = "",
  ...
  if (!is.null(readme_file)) {
    if (!file.exists(readme_file)) {
      cli_abort("{.arg readme_file} does not exist: {.file {readme_file}}")
    }
  }
  ...
  if (print_progress & !quiet) cli_progress_bar("Collecting subdirectories", total = total_subdirs)
  ...
  for (i in seq_along(subdirs)) {
    ...
    if (print_progress & !quiet) cli_progress_update()
  }

  if (print_progress & !quiet) cli_progress_done()

  # If readme_file is provided, update it
  if (!is.null(readme_file)) {
    if (!quiet) {
      cli_inform("Creating README file...")
    }
  ...
  if (length(temp_subdirs) > 0) {
    ...
    if (!is.null(readme_file)) {
      if (!quiet) {
        cli_inform("Creating MANIFEST.txt...")
      }
    ...
    if (!quiet) {
      cli_inform("Creating zip archive...")
    }
    ...
    if (!quiet) {
      cli_alert_success("Zip archive created: {.file {normalizePath(zip_filename, winslash = '/')}}")
    }
    ...
    if (split_zip && zip_file_size > max_size) {
      if (!quiet) {
        cli_inform("{.file {zip_filename}} is larger than {max_size} MB. Splitting the zip file into smaller parts...")
      }
    ...
    cli_inform("No directories with {.file .png} files found.")
  }
