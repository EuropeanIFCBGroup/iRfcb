...
#' (Sosik and Olson 2007). Biovolumes are converted to carbon according to Menden-Deuer and Lessard 2000
...
#'
#' Users can optionally provide a **custom classification** by supplying a vector of image filenames
...
#' }
#'
#' If `custom_images` and `custom_classes` are provided, the function will summarize biovolume and carbon content
#' using the manually supplied classification (e.g. results from a CNN model) instead of relying on MATLAB or manual annotation files.
#'
