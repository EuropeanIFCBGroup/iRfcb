utils::globalVariables(c("biovolume", "roi", "roi_number", "Biovolume", "chain_count", "cell_count"))
...
#' @param use_chain_counts Logical. If `TRUE`, reads the optional per-ROI `chain_count`
#'        data stored by the diatom chain counter in `.h5`/`.csv` classification files and
#'        adds `chain_count` (raw) and `cell_count` (resolved abundance) columns to the
#'        output. Only supported with automated `class_files`; not with manual files,
#'        `.mat` files, or `custom_images`. Default: `FALSE`.
#' @param single_cell_values Integer vector of `chain_count` values that should be treated
#'        as a single cell when resolving `cell_count`. Default is `c(-1, 0)`, i.e. ROIs that
#'        were not counted (`-1`) and ROIs where no cells were detected (`0`) each count as one
#'        cell. Values not listed are used verbatim. Only used when `use_chain_counts = TRUE`.
...
#' - `chain_count`, `cell_count` (only when `use_chain_counts = TRUE`): the raw per-ROI
#'   chain count and the resolved number of cells used for abundance.
#'
#' @details
...
#' @export
#'
#' @seealso \code{\link{ifcb_read_features}} \code{\link{ifcb_is_diatom}} \code{\link{ifcb_summarize_chain_counts}} \url{https://github.com/nodc-sweden/ifcb-pytorch-classify} \url{https://www.marinespecies.org/}
ifcb_extract_biovolumes <- function(feature_files, class_files = NULL, custom_images = NULL, custom_classes = NULL,
  ...
  if (!is.null(class_files)) {
    ...
    if (use_chain_counts && is_manual) {
      cli_abort(c(
        "{.arg use_chain_counts = TRUE} is not supported for manually annotated files.",
        "i" = "Chain-count data is only stored in automated {.file .h5} and {.file .csv} classification files."
      ))
    }
  }

  if (use_chain_counts && !is.null(custom_images)) {
    cli_abort(c(
      "{.arg use_chain_counts = TRUE} cannot be combined with {.arg custom_images}/{.arg custom_classes}.",
  ...
  if (!is.null(custom_images) && !is.null(custom_classes)) {
    ...
    if (is_manual) {
      ...
      for (i in seq_along(matching_class_files)) {
        ...
        tb_list[[i]] <- tibble(
          ...
          class = if (threshold == "opt")
            ...
            unlist(temp$TBclass),
          chain_count = if (use_chain_counts) {
            if (is.null(temp$chain_count)) NA_integer_ else temp$chain_count
          } else {
            NULL
          }
        )
      ...
      if (use_chain_counts) {
        if (!any(has_chain)) {
          cli_abort(c(
            "{.arg use_chain_counts = TRUE} but none of the classification files contain chain-count data.",
  ...
  if (use_chain_counts) {
    biovolume_df$cell_count <- resolve_cell_counts(biovolume_df$chain_count, single_cell_values)
  }
