...
function before calling functions that require Python.
...
- **Description**: The `USE_IRFCB_PYTHON` environment variable controls
  ...
  venv (e.g. using
  [`ifcb_py_install()`](https://europeanifcbgroup.github.io/iRfcb/reference/ifcb_py_install.md))
  when the package is loaded. The optional `IRFCB_PYTHON_VENV` variable
  controls *which* venv is activated.
- **Default**: By default, neither variable is set. This means that the
  Python environment will not be loaded automatically, and the user must
  call the
  [`ifcb_py_install()`](https://europeanifcbgroup.github.io/iRfcb/reference/ifcb_py_install.md)
  function manually before using a Python feature.
- **Usage**: To enable automatic setup of the Python environment when
  `iRfcb` is loaded, set `USE_IRFCB_PYTHON` to `"TRUE"`. By default the
  package activates the first available venv named `iRfcb` found in
  ...
  To load a specific environment instead, also set `IRFCB_PYTHON_VENV`
  to either the **name** of a venv under
  [`reticulate::virtualenv_root()`](https://rstudio.github.io/reticulate/reference/virtualenv-tools.html)
  or a **full path** to a venv directory. If `IRFCB_PYTHON_VENV` is set
  but cannot be resolved, no environment is activated (auto-discovery is
  not attempted). Both variables must be set *before* `iRfcb` is loaded.

#### How to Set These Variables

You can set the variables in your R session or make them persistent
across sessions:

1.  **Temporary for the session**: Set the variables before loading
    `iRfcb`:
...
2.  **Permanent across sessions**: To ensure these settings persist
    across R sessions, add them to your `.Renviron` file in your R home
    directory. You can easily edit the file using:
    ...
    This will automatically set the environment variables each time you
    start an R session.
