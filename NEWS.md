...
* `ifcb_correct_annotation()` and `ifcb_replace_mat_values()` now check their inputs and fail with a message you can act on. An out-of-range ROI number or `column_index` reports the value and the valid range instead of `subscript out of bounds`, a classlist with fewer than two columns is reported as such, and a missing input file or a `.mat` file without a `classlist` variable is named explicitly. `ifcb_adjust_classes()` likewise passes the reader's own refusal message through when it skips a file, instead of relabelling every problem "empty or corrupted".
