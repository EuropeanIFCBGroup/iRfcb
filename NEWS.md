...
* Removed `R.matlab` as a dependency. Reading `.mat` files without Python now uses the native R reader as well, which decodes MATLAB UTF-16 text correctly, so accented class or path names survive where `R.matlab::readMat()` could mangle them. The reader also refuses input it cannot represent faithfully, naming the variable and the reason, and reports a truncated file instead of reading it back quietly padded with zeros. Both matter because `ifcb_adjust_classes()` and `ifcb_correct_annotation()` write what they read back over the same file. `R.matlab` has moved to `Suggests`.
  * Note that this makes the reader stricter than 0.9.0 was, so a file that used to open may now stop with an error. `R.matlab` would read a `.mat` containing a struct, an object, a sparse or complex array, a logical array, more than two dimensions, or a MATLAB string array (as saved by `class2use = ["a" "b"]` in a recent MATLAB release), and iRfcb would then quietly write back something that was not what it read. Those files are now named and refused instead. A `.mat` written by iRfcb, MATLAB `ifcb-analysis` or the Python pipelines is unaffected; if you have a third-party file that no longer opens, `ifcb_get_mat_variable(..., use_python = TRUE)` still reads it via `SciPy`.
  * The reader also refuses a file whose declared dimensions disagree with the data it carries, rather than recycling the values to fill them out or allocating on the strength of a corrupt dimension.
  * Fixed the reader decoding two storage types wrongly. A MATLAB `int8` array read back unsigned, turning `-1` into `255` and `-128` into `128`, and a `uint32` value above 2147483647 read back negative (`4e9` as `-294967296`). MATLAB stores a numeric array in the narrowest type its values fit, so an ordinary `double` column of small negative numbers is stored as `int8` and was affected. Both now round-trip byte-for-byte.
