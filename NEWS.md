...
* Added support for the `IRFCB_PYTHON_VENV` environment variable. When `USE_IRFCB_PYTHON = "TRUE"`, you can now set `IRFCB_PYTHON_VENV` to either a named virtualenv or a full path to a venv directory to control which Python environment is activated on package load. If unset, the previous behaviour of auto-discovering a venv named `iRfcb` is retained.
