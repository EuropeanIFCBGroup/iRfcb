...
* The argument `adc_folder` is now deprecated from `ifcb_annotate_batch()` and replaced by `adc_files`.
* The argument `unclassified_id` is now deprecated from `ifcb_create_empty_manual_file()` and replaced by `classlist`.
* The argument `utm_zone` from `ifcb_is_near_land()` is now deprecated and is instead calculated from the median longitude.
...
## Deprecated
* `ifcb_summarize_png_data()` is deprecated and replaced by `ifcb_summarize_png_counts()`.
...
* Remove helper `handle_missing_positions()`.
* Deprecate arguments `manual_folder`, `feature_folder` and `class_folder` from `ifcb_count_mat_annotations()`, `ifcb_extract_biovolumes()`, `ifcb_extract_biovolumes()`, `ifcb_read_features()` and `ifcb_summarize_biovolumes()`.
...
## Minor improvements and fixes
* Add classifier name to mat extraction.
...
## Minor improvements and fixes
* Add multiple attempts to WoRMS API call.
* Update title.
...
* Minor Minor improvements and fixes.
...
## New features
* Add unit testing.
* Improve functionality of several functions.
...
## Minor improvements and fixes
* Improve NA handling, and make examples run.
