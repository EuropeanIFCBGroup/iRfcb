...
test_that("ifcb_summarize_png_data works correctly with sample data", {
  ...
  summary_sample <- ifcb_summarize_png_data(png_folder, hdr_folder, sum_level = "sample", verbose = TRUE)
  ...
  summary_class <- ifcb_summarize_png_data(png_folder, hdr_folder, sum_level = "class", verbose = TRUE)
...
test_that("ifcb_summarize_png_data handles empty directories gracefully", {
  ...
  expect_error(ifcb_summarize_png_data(empty_png_dir, hdr_folder, sum_level = "sample", verbose = TRUE), "No subdirectories found in the PNG folder")

  # Run the function with empty hdr directory and expect an error
  expect_error(ifcb_summarize_png_data(png_folder, empty_hdr_dir, sum_level = "sample", verbose = TRUE), "No HDR data found")
})
...
test_that("ifcb_summarize_png_data handles invalid directories gracefully", {
  ...
  expect_error(ifcb_summarize_png_data(invalid_png_dir, invalid_hdr_dir, sum_level = "sample", verbose = TRUE))
})
