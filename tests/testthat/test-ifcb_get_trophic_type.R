...
test_that("ifcb_get_trophic_type handles invalid inputs gracefully", {
  ...
  expect_error(ifcb_get_trophic_type(taxa_list = 123), "must be a character vector")

  # Check if the function handles non-logical print_complete_list
  expect_error(ifcb_get_trophic_type(print_complete_list = "yes"), "must be a logical value")

  # Check if the function handles NULL input without print_complete_list
  expect_error(ifcb_get_trophic_type(), "No valid input provided")

  # Check if the function handles input with print_complete_list
  expect_warning(ifcb_get_trophic_type(taxa_list = "Chaetoceros", print_complete_list = TRUE), "Both")
})
