...
# Helper to write a minimal class .h5 file, optionally with a chain_count dataset
...
write_test_class_h5 <- function(path, roi, classes, chain = NULL, extra = NULL) {
  ...
  if (!is.null(chain)) {
    f[["chain_count"]] <- as.integer(chain)
  }
...
test_that("ifcb_summarize_chain_counts computes abundance and chain-length stats", {
  ...
  res <- ifcb_summarize_chain_counts(dir, verbose = FALSE)
  ...
  # n_chains and length stats only over chain_count >= 1: {1, 5}
...
test_that("single_cell_values controls how chain_count == 0 is counted", {
  skip_if_not_installed("hdf5r")
  dir <- fresh_dir()
  write_test_class_h5(
    file.path(dir, "D20220101T000000_IFCB001_class.h5"),
  ...
  res <- ifcb_summarize_chain_counts(dir, single_cell_values = c(-1),
...
test_that("ifcb_summarize_chain_counts selects the requested stats", {
  skip_if_not_installed("hdf5r")
  dir <- fresh_dir()
  write_test_class_h5(
    file.path(dir, "D20220101T000000_IFCB001_class.h5"),
  ...
  res <- ifcb_summarize_chain_counts(dir, stats = c("mean", "sd"), verbose = FALSE)
...
test_that("ifcb_summarize_chain_counts rejects invalid stats", {
  skip_if_not_installed("hdf5r")
  dir <- fresh_dir()
  write_test_class_h5(
    file.path(dir, "D20220101T000000_IFCB001_class.h5"),
  ...
  expect_error(ifcb_summarize_chain_counts(dir, stats = "average", verbose = FALSE),
               "Invalid value")
})

test_that("ifcb_summarize_chain_counts aborts when no file has chain-count data", {
  skip_if_not_installed("hdf5r")
  dir <- fresh_dir()
  write_test_class_h5(
    file.path(dir, "D20220101T000000_IFCB001_class.h5"),
  ...
  expect_error(ifcb_summarize_chain_counts(dir, verbose = FALSE),
               "chain-count data")
})
...
test_that("read_class_file surfaces extra_datasets verbatim and skips missing ones", {
  ...
  expect_false(is.null(default$chain_count))
...
test_that("ifcb_summarize_chain_counts reads chain_count from CSV files", {
  dir <- fresh_dir()
  csv <- data.frame(
